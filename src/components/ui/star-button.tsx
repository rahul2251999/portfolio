...
export function StarButton({
  ...
  return (
    <button
      ...
      ref={pathRef}
      className={cn(
        "group/star-button relative inline-flex h-10 items-center justify-center gap-2 overflow-hidden rounded-3xl px-4 py-2 text-sm font-medium text-accent-white transition-colors disabled:pointer-events-none disabled:opacity-50",
        className,
      )}
      {...props}
    >
      <div
        className="pointer-events-none absolute inset-0 z-[1] aspect-square animate-star-btn bg-[radial-gradient(ellipse_at_center,var(--light-color),transparent,transparent)]"
        style={
          {
            offsetPath: "var(--path)",
            offsetDistance: "0%",
      ...
      />
      <div
        className="pointer-events-none absolute inset-0 z-[2] overflow-hidden rounded-[inherit] border border-black/10 text-white dark:border-white/15 dark:text-black"
        style={{ borderWidth: "var(--border-width)" }}
        aria-hidden="true"
      >
        <StarBackground color={backgroundColor} />
      </div>
      <span className="relative z-[3] inline-block bg-gradient-to-t from-accent-white to-accent-gray bg-clip-text text-transparent dark:from-white dark:to-neutral-500">
        {children}
      </span>
    </button>
  );
